...
- (void) makeVasApiCall{
    ...
    if(HASH_KEY_GENERATION_FROM_SERVER){
        checkSum = [[[SharedDataManager sharedDataManager] hashDict] valueForKey:MOBIL_SDK];
    }
    else{
        ViewController *viewController = [[ViewController alloc] init];
        checkSum = [viewController createCheckSumString:[NSString stringWithFormat:@"%@|%@|%@|%@",[[[SharedDataManager sharedDataManager] allInfoDict] objectForKey:PARAM_KEY],PARAM_VAS_COMMAND_VALUE,DEFAULT,[[[SharedDataManager sharedDataManager] allInfoDict] objectForKey:PARAM_SALT]]];
    }
